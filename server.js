...
app.get('/api', (req, res) => {
  res.json({
    ...
    version:   '1.0.0',
    endpoints: {
      'POST /auth/register':   'Registrar usuario { "email": "...", "password": "..." } — contraseña hasheada con bcrypt',
      'POST /auth/login':      'Iniciar sesión { "email": "...", "password": "..." } — verifica hash bcrypt',
...
// ══════════════════════════════════════════
//  RUTAS AUTENTICACIÓN (con bcrypt)
// ══════════════════════════════════════════

// POST /auth/register — registrar usuario con contraseña hasheada
app.post('/auth/register', async (req, res) => {
  const { email, password } = req.body;
  if (!email || !password) {
    return res.status(400).json({ ok: false, error: 'Faltan campos email o password' });
  }
...
// POST /auth/login — verificar contraseña con bcrypt
app.post('/auth/login', async (req, res) => {
  ...
  try {
    const data = await supabase(
      `usuarios?email=eq.${encodeURIComponent(email)}&select=id,email,password`
    );
    if (!data || !data.length) {
