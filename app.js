...
// ══════════════════════════════════════════
//  PRUEBA DE ENDPOINTS — Sección API REST
// ══════════════════════════════════════════
async function probarEndpoint(ruta) {
  const el = document.getElementById('api-result');
  el.style.color = '#94a3b8';
  el.textContent = `⏳ GET ${ruta} ...`;
  try {
    const res  = await fetch(`${API_BASE}${ruta}`);
    const data = await res.json();
    el.style.color = res.ok ? '#38bdf8' : '#ef4444';
    el.textContent = JSON.stringify(data, null, 2);
  } catch (e) {
    el.style.color = '#ef4444';
    el.textContent = `❌ Error: ${e.message}\n\nAsegúrate de que el servidor Express esté corriendo:\n  node server.js`;
  }
}

async function probarPost() {
  const val = document.getElementById('api-post-val').value;
  const el  = document.getElementById('api-result');
  if (!val) { el.textContent = '⚠️ Ingresa un valor entre 0 y 4095'; return; }
  el.style.color = '#94a3b8';
  el.textContent = `⏳ POST /sensor { valor_ldr: ${val} } ...`;
  try {
    const res  = await fetch(`${API_BASE}/sensor`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ valor_ldr: Number(val) })
    });
    const data = await res.json();
    el.style.color = res.ok ? '#10b981' : '#ef4444';
    el.textContent = JSON.stringify(data, null, 2);
    if (res.ok) fetchData(); // refresca la gráfica con el nuevo dato
  } catch (e) {
    el.style.color = '#ef4444';
    el.textContent = `❌ Error: ${e.message}\n\nAsegúrate de que el servidor Express esté corriendo:\n  node server.js`;
  }
...
async function register() {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok || !data.ok) {
      errEl.textContent = data.error?.includes('duplicate')
  ...
  try {
    ...
    realtimeSocket.onopen = () => {
      ...
      realtimeSocket.send(JSON.stringify({
        ...
        payload: { event: 'INSERT', schema: 'public', table: 'sensores' },
        ref: '2'
      }));
...
async function enviarInforme() {
  ...
  try {
    ...
    if (errMsg.includes('412') || errMsg.includes('template')) {
      statusEl.textContent = `❌ Error en el template de EmailJS. El campo "To" debe ser {{to_email}}. Detalle: ${errMsg}`;
    } else {
